...
public class Union implements LogicalPlan {
    ...
    static Builder create(UnionSelect ttr) {
        return (tableStats, usedColsByParent) -> {
            ...
            LogicalPlan lhsPlan = LogicalPlanner.plan(left, FetchMode.NEVER_CLEAR, false).build(tableStats, usedFromLeft);
            LogicalPlan rhsPlan = LogicalPlanner.plan(right, FetchMode.NEVER_CLEAR, false).build(tableStats, usedFromRight);

            return new Union(lhsPlan, rhsPlan);
        };
    }
    ...
    private Union(LogicalPlan lhs, LogicalPlan rhs) {
        this.lhs = lhs;
        this.rhs = rhs;
    ...
    public Plan build(Planner.Context plannerContext,
        ...
        Plan left = lhs.build(plannerContext, projectionBuilder, limit + offset, offset, null, childPageSizeHint);
        Plan right = rhs.build(plannerContext, projectionBuilder, limit + offset, offset, null, childPageSizeHint);
    ...
    public LogicalPlan tryCollapse() {
        ...
        if (lhs != lhsCollapsed || rhs != rhsCollapsed) {
            return new Union(lhsCollapsed, rhsCollapsed);
        }
        return this;
    }
     ...
     */
    private static Plan addMergeIfNeeded(Plan plan, Planner.Context plannerContext) {
